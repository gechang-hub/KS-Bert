...
with tf.variable_scope('accuracy'):
    correct_predictions = tf.equal(predict, tf.argmax(input_y, 1))#pre
...
with tf.variable_scope('f1'):
    ...
    # accuracy = tf.reduce_mean(tf.cast(correct_predictions, 'float32'), name='f1')
    model_pred = predict
    model_True = tf.argmax(input_y, 1)
    # f1 = f1_score(model_pred,model_True)
